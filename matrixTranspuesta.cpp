...
// transpone la matriz
vector<vector<int>> transponerMatriz(const vector<vector<int>>& matrix_B) {
    int filas = matrix_B.size();
    int columnas = matrix_B[0].size();
    vector<vector<int>> B_transpose(columnas, vector<int>(filas));
...
{   
	...
	vector<vector<int>> matrix_B_transpose = transponerMatriz(matrix_B);
    ...
    for(int i = 0; i < row_1; i++){
        for(int j = 0; j < col_2; j++){
            for(int k = 0; k < col_1; k++){
                result_matrix[i][j] += matrix_A[i][k] * matrix_B_transpose[j][k];
            }
        }
    }
